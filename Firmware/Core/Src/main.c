...
#define ADC_RESULT_LEN 64
#define ADC_BUF_LEN OVR*ADC_RESULT_LEN
#define UART_TX_BUF_LEN ADC_RESULT_LEN*2
...
//uint16_t adcResult[ADC_RESULT_LEN];
uint8_t uartTxBuffer[UART_TX_BUF_LEN];
...
//	x;
	...
	while (1) {
		...
		if (uartTxRequest && !uartTxBusy) {
			uartTxRequest = 0;
			uartTxBusy = 1;
			HAL_UART_Transmit_DMA(&huart1, uartTxBuffer, UART_TX_BUF_LEN);
		}
	}
	/* USER CODE END 3 */
}
...
static inline void processAdcBlock(uint16_t *buffer, uint16_t len) {
	for (uint16_t i = 0; i < len; i += OVR) {
		...
		for (uint8_t j = 0; j < OVR; ++j) {
			adcResultTemp += buffer[i + j];
		}
		adcResultTemp >>= P;
		if (uartTxIndex + 1 < UART_TX_BUF_LEN) {
			uartTxBuffer[uartTxIndex++] = adcResultTemp >> 8;
			uartTxBuffer[uartTxIndex++] = adcResultTemp;
		}
	}
	if (uartTxIndex >= UART_TX_BUF_LEN && !uartTxRequest) {
		uartTxRequest = 1;
	}
}
...
void HAL_ADC_ConvHalfCpltCallback(ADC_HandleTypeDef *hadc) {
	if (hadc->Instance == ADC1) {
		processAdcBlock(&adcBuffer[0], ADC_BUF_LEN / 2);
	}
}

void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc) {
	if (hadc->Instance == ADC1) {
		processAdcBlock(&adcBuffer[ADC_BUF_LEN / 2], ADC_BUF_LEN / 2);
	}
}
